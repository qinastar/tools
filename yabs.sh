...
if [[ -z "$IPV4_CHECK" && -z "$IPV6_CHECK" ]]; then
	echo -e
	echo -e "警告: 未检测到IPv4或IPv6连接。检查DNS问题..."
fi
...
echo -e "基本系统信息:"
echo -e "---------------------------------"
UPTIME=$(uptime | awk -F'( |,|:)+' '{d=h=m=0; if ($7=="min") m=$6; else {if ($7~/^day/) {d=$6;h=$8;m=$9} else {h=$6;m=$7}}} {print d+0,"天,",h+0,"小时,",m+0,"分钟"}')
echo -e "运行时间   : $UPTIME"
...
echo -e "CPU核心数  : $CPU_CORES @ $CPU_FREQ"
CPU_AES=$(grep aes /proc/cpuinfo)
[[ -z "$CPU_AES" ]] && CPU_AES="\xE2\x9D\x8C 未启用" || CPU_AES="\xE2\x9C\x94 已启用"
echo -e "AES-NI     : $CPU_AES"
CPU_VIRT=$(grep 'vmx\|svm' /proc/cpuinfo)
[[ -z "$CPU_VIRT" ]] && CPU_VIRT="\xE2\x9D\x8C 未启用" || CPU_VIRT="\xE2\x9C\x94 已启用"
...
if [[ -n $JSON ]]; then
	...
	AES=$([[ "$CPU_AES" = *已启用* ]] && echo "true" || echo "false")
	CPU_VIRT_BOOL=$([[ "$CPU_VIRT" = *已启用* ]] && echo "true" || echo "false")
...
function launch_geekbench {
	...
	if [[ $GB_RUN == *True* ]]; then # run GB test
		...
		if [ -z "$GEEKBENCH_TEST" ]; then
			...
			printf "%-15s | %-30s\n" "测试项目" "数值"
			printf "%-15s | %-30s\n" "" ""
			printf "%-15s | %-30s\n" "单核得分" "$GEEKBENCH_SCORES_SINGLE"
			printf "%-15s | %-30s\n" "多核得分" "$GEEKBENCH_SCORES_MULTI"
			printf "%-15s | %-30s\n" "完整测试" "$GEEKBENCH_URL"
...
function calculate_time_taken() {
	...
	if [ ${time_taken} -gt 60 ]; then
		min=$(( time_taken / 60 ))
		sec=$(( time_taken % 60 ))
		echo "YABS 测试完成，用时 ${min} 分 ${sec} 秒"
	else
		echo "YABS 测试完成，用时 ${time_taken} 秒"
	fi
